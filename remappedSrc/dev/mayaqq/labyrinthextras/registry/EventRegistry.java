...
public class EventRegistry {
    public static void register() {
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            Bot.discordBot.getGuilds().forEach(guild -> {
                guild.getChannelById(TextChannel.class, LabyrinthExtrasConfig.CONFIG.botChannel).sendMessage("Připojil se hráč: **" + player.getDisplayName().getString() + "**!").queue();
            });
        ...
        ServerPlayConnectionEvents.DISCONNECT.register((handler, server) -> {
            ...
            if (Bot.discordBot == null) return;
            Bot.discordBot.getGuilds().forEach(guild -> {
                guild.getChannelById(TextChannel.class, LabyrinthExtrasConfig.CONFIG.botChannel).sendMessage("Odpojil se hráč: **" + player.getDisplayName().getString() + "**!").queue();
            });
            Bot.discordBot.getPresence().setActivity(Activity.playing("S " + (server.getPlayerManager().getPlayerList().size() + 1) + "/" + server.getPlayerManager().getMaxPlayerCount() + " hráči!"));
        });
